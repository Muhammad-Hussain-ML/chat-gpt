...
# --- File Upload ---
uploaded_file = st.file_uploader("📎 Upload an image or document (optional)", type=["jpg", "jpeg", "png", "pdf", "txt"])

file_part = None
if uploaded_file:
    file_part = Part.from_data(data=uploaded_file.read(), mime_type=uploaded_file.type)
    ...
    with st.chat_message("model"):
        try:
            ...
            for chunk in response:
                ...
                st.markdown(output + "▌")
            st.markdown(output)
            st.session_state.chat_history.append({"role": "model", "text": output})
